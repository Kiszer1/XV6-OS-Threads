...
exit(int status)
{
  ...
  // Making sure were the only thread in exit.
  acquire(&p->lock);
  if (p->exited != 1) {
    release(&p->lock);
    kthread_exit(status);
  ...
  for (kt = p->kthread; kt < &p->kthread[NKT]; kt ++) {
    if (kt != mykthread() && kt!= 0 && kt->state != KTUNUSED && kt->state != KTZOMBIE) {
      ktid = kt->ktid;
      kthread_kill(ktid);
      kthread_join(ktid, 0);
    }
...
wait(uint64 addr)
{
  ...
  for(;;){
    ...
    if(!havekids || killed(p) || mykthread()->killed != 0){
      release(&wait_lock);
      return -1;
    }
...
wakeup(void *chan)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    for (kt = p->kthread; kt < &p->kthread[NKT]; kt++) {
      if (kt != mykthread()) {
        acquire(&kt->lock);
        if (kt->state == KTSLEEPING && kt->chan == chan) {
          kt->state = KTRUNNABLE;
        }
        release(&kt->lock);
      }
